...
class course_renderer extends \core_course_renderer {
    ...
    public function _ccnActivityNav_course_section_cm_list($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        $sectionoutput = '';
        if ($modulehtml = $this->_ccnActivityNav_course_default_section_cm_list_item($course,
            'About', $sectionreturn, $displayoptions)) {
            $sectionoutput .= $modulehtml;
        }
        if ($modulehtml = $this->_ccnActivityNav_course_default_section_cm_list_item($course,
            'Content', $sectionreturn, $displayoptions)) {
            $sectionoutput .= $modulehtml;
        }
        ...
        if (isloggedin() && !isguestuser()) {
            if (is_enrolled($coursecontext, $USER, '', true)) {
                if ($modulehtml = $this->_ccnActivityNav_course_default_section_cm_list_item($course,
                    'Timeline', $sectionreturn, $displayoptions)
                ) {
                    $sectionoutput .= $modulehtml;
                }
    ...
    public function _ccnActivityNav_course_section_cm_list_item($course, &$completioninfo, cm_info $mod, $sectionreturn, $displayoptions = array()) {
        ...
        if ($modulehtml = $this->_ccnActivityNav_course_section_cm($course, $completioninfo, $mod, $sectionreturn, $displayoptions)) {
            ...
            if(strpos($ccnUriForCourseFocus, $ccnUri) !== false) {
                $modclasses .= ' active';
            }
     ...
     * @return String
     */
    public function _ccnActivityNav_course_default_section_cm_list_item($course, $mod, $sectionreturn, $displayoptions = array()) {
        $ccnUriForCourseFocus = $_SERVER['REQUEST_URI'];
        
        $output = '';
        $mod_id = str_replace(" ", "-", strtolower($mod));
        if ($modulehtml = $this->_ccnActivityNav_course_default_section_cm($course, $mod, $sectionreturn, $displayoptions)) {
            $ccnUri = '/course/view.php?id='.$course->id;
            if ($mod_id != "about") $ccnUri .= '&tab='.$mod_id;
            $modclasses = 'activity ' . $mod_id . ' modtype_' . $mod_id;
            if($ccnUriForCourseFocus == $ccnUri) {
                $modclasses .= ' active';
            }
            $output .= html_writer::tag('li', $modulehtml, array('class' => $modclasses, 'id' => 'module-' . $mod_id));
        }
        return $output;
    }
     ...
     * @return string
     */
    public function _ccnActivityNav_course_default_section_cm($course, $mod, $sectionreturn, $displayoptions = array()) {
        $output = '';
        ...
        // Display the link to the module (or do nothing if module has no url)
        $modid = str_replace(" ", "-", strtolower($mod));
        
        if (!empty($mod)) {
            // Start the div for the activity title, excluding the edit icons.
            $output .= html_writer::start_tag('div', array('class' => 'activityinstance'));
            $modurl = new moodle_url('/course/view.php?id='.$course->id.($modid != 'about' ? '&tab='.$modid : ''));
            $output .= '<a class="aalink" onclick="" href="'.$modurl.'">
                <span class="instancename">'.$mod.'</span>
            </a>';
     ...
     * @return string
     */
    public function get_course_enrolment($course) {
        global $PAGE, $OUTPUT;
        
        $courseContext = context_course::instance($course->id);
        $course_enrolments = count_enrolled_users($courseContext);
        $renderEnrolmentIcon = '<div class="course-followers"><div class="course-followers-content">';
        if($PAGE->theme->settings->coursecat_enrolments != 1) {
            if ($PAGE->theme->settings->courseicon_followers) {
                $enrolmentIcon = '';
                if (method_exists('theme_edumy\output\core_renderer', 'get_theme_image_courseicon_followers') && method_exists('theme_edumy\output\core_renderer_maintenance', 'get_theme_image_courseicon_followers') && !empty($OUTPUT->get_theme_image_courseicon_followers())) {
                    $enrolmentIcon = $OUTPUT->get_theme_image_courseicon_followers(null, 100);
                }
            }
            if ($enrolmentIcon) {
                $renderEnrolmentIcon .= '<img class="fluid-img" src="'.$enrolmentIcon.'"><span>' . $course_enrolments . ' ' . (($course_enrolments > 1) ? get_string('followers', 'theme_edumy') : get_string('follower', 'theme_edumy')) . '</span>';
            } else {
                $renderEnrolmentIcon .= '<i class="flaticon-profile"></i><span>' . $course_enrolments . ' ' . (($course_enrolments > 1) ? get_string('followers', 'theme_edumy') : get_string('follower', 'theme_edumy')) . '</span>';
            }
        }
        $renderEnrolmentIcon .= '</div></div>';
        return $renderEnrolmentIcon;
    }
     ...
     * @return string
     */
    public function get_course_config_icon($type) {
        global $PAGE, $OUTPUT;
        
        $courseIcon = '';
        $renderCourseIcon = '';
        if ($type == "book") {
            if ($PAGE->theme->settings->courseicon_book) {
                if (method_exists('theme_edumy\output\core_renderer', 'get_theme_image_courseicon_book') && method_exists('theme_edumy\output\core_renderer_maintenance', 'get_theme_image_courseicon_book') && !empty($OUTPUT->get_theme_image_courseicon_book())) {
                    $courseIcon = $OUTPUT->get_theme_image_courseicon_book(null, 100);
                }
            }
            if ($courseIcon) {
                $renderCourseIcon .= '<img class="fluid-img" src="'.$courseIcon.'">';
            } else {
                $renderCourseIcon .= '<i class="flaticon-book"></i>';
            }
        } else if ($type == "chat") {
            if ($PAGE->theme->settings->courseicon_chat) {
                if (method_exists('theme_edumy\output\core_renderer', 'get_theme_image_courseicon_chat') && method_exists('theme_edumy\output\core_renderer_maintenance', 'get_theme_image_courseicon_chat') && !empty($OUTPUT->get_theme_image_courseicon_chat())) {
                    $courseIcon = $OUTPUT->get_theme_image_courseicon_chat(null, 100);
                }
            }
            if ($courseIcon) {
                $renderCourseIcon .= '<img class="fluid-img" src="'.$courseIcon.'">';
            } else {
                $renderCourseIcon .= '<i class="flaticon-comment"></i>';
            }
        } else if ($type == "download") {
            if ($PAGE->theme->settings->courseicon_download) {
                if (method_exists('theme_edumy\output\core_renderer', 'get_theme_image_courseicon_download') && method_exists('theme_edumy\output\core_renderer_maintenance', 'get_theme_image_courseicon_download') && !empty($OUTPUT->get_theme_image_courseicon_download())) {
                    $courseIcon = $OUTPUT->get_theme_image_courseicon_download(null, 100);
                }
            }
            if ($courseIcon) {
                $renderCourseIcon .= '<img class="fluid-img" src="'.$courseIcon.'">';
            } else {
                $renderCourseIcon .= '<i class="flaticon-download-1"></i>';
            }
        } else if ($type == "followers") {
            if ($PAGE->theme->settings->courseicon_followers) {
                if (method_exists('theme_edumy\output\core_renderer', 'get_theme_image_courseicon_followers') && method_exists('theme_edumy\output\core_renderer_maintenance', 'get_theme_image_courseicon_followers') && !empty($OUTPUT->get_theme_image_courseicon_followers())) {
                    $courseIcon = $OUTPUT->get_theme_image_courseicon_followers(null, 100);
                }
            }
            if ($courseIcon) {
                $renderCourseIcon .= '<img class="fluid-img" src="'.$courseIcon.'">';
            } else {
                $renderCourseIcon .= '<i class="flaticon-profile"></i>';
            }
        } else if ($type == "plus") {
            if ($PAGE->theme->settings->courseicon_plus) {
                if (method_exists('theme_edumy\output\core_renderer', 'get_theme_image_courseicon_plus') && method_exists('theme_edumy\output\core_renderer_maintenance', 'get_theme_image_courseicon_plus') && !empty($OUTPUT->get_theme_image_courseicon_plus())) {
                    $courseIcon = $OUTPUT->get_theme_image_courseicon_plus(null, 100);
                }
            }
            if ($courseIcon) {
                $renderCourseIcon .= '<img class="fluid-img" src="'.$courseIcon.'">';
            } else {
                $renderCourseIcon .= '<i class="flaticon-add-contact"></i>';
            }
        } else if ($type == "share") {
            if ($PAGE->theme->settings->courseicon_share) {
                if (method_exists('theme_edumy\output\core_renderer', 'get_theme_image_courseicon_share') && method_exists('theme_edumy\output\core_renderer_maintenance', 'get_theme_image_courseicon_share') && !empty($OUTPUT->get_theme_image_courseicon_share())) {
                    $courseIcon = $OUTPUT->get_theme_image_courseicon_share(null, 100);
                }
            }
            if ($courseIcon) {
                $renderCourseIcon .= '<img class="fluid-img" src="'.$courseIcon.'">';
            } else {
                $renderCourseIcon .= '<i class="flaticon-share"></i>';
            }
        }
        return $renderCourseIcon;
    }
}