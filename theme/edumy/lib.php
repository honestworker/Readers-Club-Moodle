...
function theme_edumy_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM && ( $filearea === 'headerlogo1' ||
                                                      ...
                                                      $filearea === 'footer_row_1_col_2_icon_1' ||
                                                      $filearea === 'footer_row_1_col_2_icon_2' ||
                                                      $filearea === 'footer_row_1_col_2_icon_3' ||
                                                      $filearea === 'footer_row_1_col_3_icon_1' ||
                                                      $filearea === 'footer_row_1_col_3_icon_2' ||
                                                      $filearea === 'coursebadge_common' ||
                                                      $filearea === 'coursebadge_enrolled' ||
                                                      $filearea === 'courseicon_book' ||
                                                      $filearea === 'courseicon_chat' ||
                                                      $filearea === 'courseicon_download' ||
                                                      $filearea === 'courseicon_followers' ||
                                                      $filearea === 'courseicon_plus' ||
                                                      $filearea === 'courseicon_return' ||
                                                      $filearea === 'courseicon_share'
                                                    )) {
...
function theme_edumy_process_css($css, $theme) {
    ...
    $setting = $theme->settings->course_category_first_color;
    $tag = '[[setting:course_category_first_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#9E00FF';}
    $css = str_replace($tag, $replacement, $css);
    
    $setting = $theme->settings->course_category_second_color;
    $tag = '[[setting:course_category_second_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#0A47C9';}
    $css = str_replace($tag, $replacement, $css);
    
    $setting = $theme->settings->course_category_third_color;
    $tag = '[[setting:course_category_third_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#9E00FF';}
    $css = str_replace($tag, $replacement, $css);
    
    $setting = $theme->settings->course_first_color;
    $tag = '[[setting:course_first_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#FEF445';}
    $css = str_replace($tag, $replacement, $css);

    $setting = $theme->settings->course_second_color;
    $tag = '[[setting:course_second_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#12CDD4';}
    $css = str_replace($tag, $replacement, $css);

    $setting = $theme->settings->course_third_color;
    $tag = '[[setting:course_third_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#0CA789';}
    $css = str_replace($tag, $replacement, $css);

    $setting = $theme->settings->course_forth_color;
    $tag = '[[setting:course_forth_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#FE9928';}
    $css = str_replace($tag, $replacement, $css);

    $setting = $theme->settings->course_fifth_color;
    $tag = '[[setting:course_fifth_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#3499ED';}
    $css = str_replace($tag, $replacement, $css);

    $setting = $theme->settings->course_sixth_color;
    $tag = '[[setting:course_sixth_color]]';
    $replacement = $setting;
    if(is_null($replacement)){$replacement = '#DA0063';}
    $css = str_replace($tag, $replacement, $css);
    ...
    if($theme->settings->primary_font == 0) {
        $font_name = 'Open Sans';
        $font_src = 'https://fonts.googleapis.com/css2?family=Open+Sans:wght@300;400;600;700;800&display=swap';
    } else if($theme->settings->primary_font == 1){
        $font_name = 'Josefin Sans';
        $font_src = 'https://fonts.googleapis.com/css2?family=Josefin+Sans:wght@100;200;300;400;500;600;700&display=swap';
    } else if($theme->settings->primary_font == 2){
        $font_name = 'Nunito';
        $font_src = 'https://fonts.googleapis.com/css?family=Nunito:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 3){
        $font_name = 'Dosis';
        $font_src = 'https://fonts.googleapis.com/css?family=Dosis:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 4){
        $font_name = 'Lato';
        $font_src = 'https://fonts.googleapis.com/css?family=Lato:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 5){
        $font_name = 'Maven Pro';
        $font_src = 'https://fonts.googleapis.com/css?family=Maven+Pro:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 6){
        $font_name = 'Montserrat';
        $font_src = 'https://fonts.googleapis.com/css?family=Montserrat:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 7){
        $font_name = 'Playfair Display';
        $font_src = 'https://fonts.googleapis.com/css?family=Playfair+Display:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 8){
        $font_name = 'Poppins';
        $font_src = 'https://fonts.googleapis.com/css?family=Poppins:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 9){
        $font_name = 'Raleway';
        $font_src = 'https://fonts.googleapis.com/css?family=Raleway:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 10){
        $font_name = 'Roboto';
        $font_src = 'https://fonts.googleapis.com/css?family=Roboto:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 11){
        $font_name = 'Lora';
        $font_src = 'https://fonts.googleapis.com/css?family=Lora:400,500,600,700&display=swap';
    } else if($theme->settings->primary_font == 12){
        $font_name = 'Oxygen';
        $font_src = 'https://fonts.googleapis.com/css?family=Oxygen:400,500,600,700&display=swap';
    } else {
        $font_name = 'Open Sans';
        $font_src = 'https://fonts.googleapis.com/css2?family=Open+Sans:wght@300;400;600;700;800&display=swap';
    }
    ...
    if(is_null($replacement_src)){$replacement_src = '@import url("https://fonts.googleapis.com/css?family=Open+Sans:wght@300;400;600;700;800&display=swap");';}
    $css = str_replace($tag_src, $replacement_src, $css);
    
    if($theme->settings->secondary_font == 0) {
        $font_name = 'Open Sans';
        $font_src = 'https://fonts.googleapis.com/css2?family=Open+Sans:wght@300;400;600;700;800&display=swap';
    } else if($theme->settings->secondary_font == 1){
        $font_name = 'Josefin Sans';
        $font_src = 'https://fonts.googleapis.com/css2?family=Josefin+Sans:wght@100;200;300;400;500;600;700&display=swap';
    ...
    $replacement_src = $font_src;
    if(is_null($replacement_src)){$replacement_src = '@import url("https://fonts.googleapis.com/css2?family=Josefin+Sans:wght@100;200;300;400;500;600;700&display=swap");';}
    $css = str_replace($tag_src, $replacement_src, $css);
