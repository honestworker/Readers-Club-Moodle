...
class ccnCourseHandler {
    public function ccnGetCourseDetails($courseId) {
        ...
        if ($DB->record_exists('course', array('id' => $courseId))) {
            ...
            $courseAgeRange = $courseRecord->agerange;
            ...
            $ccnCourse->ageRange = $courseAgeRange;
            ...
            $ccnRender->enrolmentIcon2  = '';
            $ccnRender->enrolmentIcon3  = '';
            if($PAGE->theme->settings->coursecat_enrolments != 1) {
                if ($PAGE->theme->settings->courseicon_followers) {
                    ...
                    if(method_exists('theme_edumy\output\core_renderer', 'get_theme_image_courseicon_followers') && method_exists('theme_edumy\output\core_renderer_maintenance', 'get_theme_image_courseicon_followers') && !empty($OUTPUT->get_theme_image_courseicon_followers())){
                        $enrolmentIcon = $OUTPUT->get_theme_image_courseicon_book(null, 100);
                    }
                    $ccnRender->enrolmentIcon  = '<li class="list-inline-item"><img class="fluid-img" src="'.$enrolmentIcon.'"></li><li class="list-inline-item">' . $ccnCourse->enrolments . '</li>';
                    $ccnRender->enrolmentIcon1 = '<li class="list-inline-item"><img class="fluid-img" src="'.$enrolmentIcon.'"></li><li class="list-inline-item">' . $ccnCourse->enrolments. ' ' . get_string('students_enrolled', 'theme_edumy') . '</li>';
                    $ccnRender->enrolmentIcon2 = '<li class="list-inline-item"><img class="fluid-img" src="'.$enrolmentIcon.'"></li><li class="list-inline-item">' . $ccnCourse->enrolments . ' ' . (($ccnCourse->enrolments > 1) ? get_string('followers', 'theme_edumy') : get_string('follower', 'theme_edumy')) . '</li>';
                    $ccnRender->enrolmentIcon3 = '<li class="list-inline-item"><img class="fluid-img" src="'.$enrolmentIcon.'"></li><li class="list-inline-item">' . $ccnCourse->enrolments . '</li>';
                } else {
                    ...
                    $ccnRender->enrolmentIcon2 = '<li class="list-inline-item"><i class="flaticon-profile"></i></li><li class="list-inline-item">' . $ccnCourse->enrolments . ' ' . (($ccnCourse->enrolments > 1) ? get_string('followers', 'theme_edumy') : get_string('follower', 'theme_edumy')) . '</li>';
                    $ccnRender->enrolmentIcon3 = '<li class="list-inline-item"><i class="flaticon-profile"></i></li><li class="list-inline-item">' . ' ' . $ccnCourse->enrolments . '</li>';
                }
            }
            ...
            $title_max_length = 34;
            $ccnCourseFullName = $ccnCourse->fullName;
            if (strlen($ccnCourseFullName) > $title_max_length) {
                $ccnCourseFullName = substr($ccnCourseFullName, 0, $title_max_length);
            }
            $ccnRender->title              =     '<span>'. $ccnCourseFullName .'</span>';
            $ccnRender->starRating         =     $ccnRenderStars;
            //$ccnRender->coverImage        =     '<img class="img-whp" src="'. $contentimages .'" alt="'.$ccnCourse->fullName.'">';
            $ccnRender->coverImage         =     '<div class="course-img-wrapper"><a href="' . $ccnCourse->url . '"><div class="course-img" ' . ($contentimages ? 'style="background-image: url('. $contentimages . ')"' : '') . '><img src="'. $CFG->wwwroot . '/theme/edumy/images/cover_transparent.png' . '"></div></a></div>';
            /* @ccnBreak */
    ...
    public function ccnBuildCourseFilterType($ccnType) {
        ...
        if(!empty($ccnType) && in_array($ccnType, $ccnTypes)) {
    ...
    public function ccnGetCourses($limitfrom=0, $limitnum=0, $conditions=null, $sort='', $fields='*') {
        global $CFG, $DB;
        
        // todo: we DONT want site context, so we're avoiding it with limitfrom1.
        $ccnCourses = $DB->get_records('course', $conditions, $sort, $fields, $limitfrom, $limitnum);
    ...
    public function ccnGetCoursesIds($limitfrom=0, $limitnum=0, $conditions=null, $sort='', $fields='*') {
        global $CFG, $DB;
        
        $ccnCourses = $this->ccnGetCourses($limitfrom, $limitnum, $conditions, $sort, $fields);
