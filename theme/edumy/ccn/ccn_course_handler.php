  ...
  public function ccnGetCourseDetails($courseId) {
    ...
    if ($DB->record_exists('course', array('id' => $courseId))) {
      ...
      if($PAGE->theme->settings->coursecat_modified != 1){
        $ccnRender->updatedDate         =     '<p class="update-date">'.get_string('updated', 'theme_edumy').' '.userdate($courseUpdated, get_string('strftimedatefullshort', 'langconfig')).'</p>';
      }
      $ccnRender->title             =     '<a href="'. $ccnCourse->url .'"><span>'. $ccnCourse->fullName .'</span></a>';
      $ccnRender->starRating        =     $ccnRenderStars;
      //$ccnRender->coverImage        =     '<img class="img-whp" src="'. $contentimages .'" alt="'.$ccnCourse->fullName.'">';
      $ccnRender->coverImage        =     '<div class="img" style="background-image: url('. $contentimages .')"><img src="'.$CFG->wwwroot . '/theme/edumy/images/cover_transparent.png'.'"></div>';
      /* @ccnBreak */
